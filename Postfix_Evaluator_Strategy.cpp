...
/*Implementation of Postfix_Evaluator_Strategy Concrete strategy*/
...
void Postfix_Evaluator_Strategy::Convert(Stack<Command *>& temp,Array<Command *>& postfix, Command * cmd,size_t &i)
{
	if(temp.is_empty())
	{
		temp.push(cmd);
...
bool Postfix_Evaluator_Strategy::evaluate_postfix(Array_Iterator<Command *> & arr_iter, int & result)
{
	Stack <int> s;
